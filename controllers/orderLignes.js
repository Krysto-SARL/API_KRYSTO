...
exports.getOrderLigne = asyncHandler(async (req, res, next) => {
  ...
  if (!orderLigne) {
    return next(
      new ErrorResponse(`Order Ligne not found with ID of ${req.params.id}`, 404),
    )
  }
...
exports.updateOrderLigne = asyncHandler(async (req, res, next) => {
  const orderLigne = await OrderLigne.findByIdAndUpdate(req.params.id, req.body, {
    new: true,
    runValidators: true,
  })
...
exports.deleteOrderLigne = asyncHandler(async (req, res, next) => {
  ...
  if (!orderLigne) {
    return next(
      new ErrorResponse(`orderLigne not found with ID of ${req.params.id}`, 404),
    )
  }
